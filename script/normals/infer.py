...
from marigold import MarigoldNormalsOutput, MarigoldNormalsPipeline
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Marigold : Surface Normals Estimation : Dataset Inference"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    dataset: BaseNormalsDataset = get_dataset(
        cfg_data, base_data_dir=base_data_dir, mode=DatasetMode.RGB_ONLY
    )
    assert isinstance(dataset, BaseNormalsDataset)
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            np.save(save_to, normals_pred)  # [3,H,W]
